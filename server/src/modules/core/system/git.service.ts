...
export class GitService {
    ...
    constructor() {
        ...
        // Architectural Fix: Use bundled Git by default (Portable)
        const bundledGitPath = path.resolve(process.cwd(), '../tools/git/cmd/git.exe');

        // Fallback: Archive path dev environment
        const archiveGitPath = 'd:\\Antigravity\\_archive\\Git_Trash\\cmd\\git.exe';

        if (require('fs').existsSync(bundledGitPath)) {
            this.gitPath = bundledGitPath;
        } else {
            this.gitPath = archiveGitPath;
            console.warn('[GitService] Bundled Git not found, falling back to archive:', archiveGitPath);
        }

        console.log('[GitService] Initialized with repoPath:', this.repoPath);
        console.log('[GitService] Using Git executable at:', this.gitPath);
    }

    private executeGitCommand(args: string[]): Promise<string> {
        return new Promise((resolve, reject) => {
            console.log(`[GitService] Executing: ${this.gitPath} ${args.join(' ')} in ${this.repoPath}`);
            const process = spawn(this.gitPath, args, { cwd: this.repoPath });
