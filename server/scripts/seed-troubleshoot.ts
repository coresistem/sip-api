...
async function main() {
    console.log('🔧 Seeding Troubleshoot entries...\n');
    ...
    for (const entry of TROUBLESHOOT_ENTRIES) {
        try {
            ...
            console.log(`✅ ${entry.tsId}: ${entry.title}`);
        } catch (error) {
            console.error(`❌ ${entry.tsId}: Failed -`, error);
        }
    }

    console.log(`\n🎉 Done! Seeded ${TROUBLESHOOT_ENTRIES.length} entries.`);
}
