    ...
    const animate = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i; j < particles.length; j++) {
          ...
          if (distance < 100) {
            ctx.beginPath();
            // Default lines are Blue
            // If both particles are highlighted (near mouse), we could make the line gold, 
            // but the request emphasizes connecting to the cursor. 
            // Let's keep the network structure Blue for contrast.
        ...
        if (mouse.x !== -9999) {
          ...
          if (distance < 180) {
            ctx.beginPath();
            // Connection to cursor is Golden
