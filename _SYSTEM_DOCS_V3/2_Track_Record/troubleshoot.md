...
| [TS-049](#ts-049) | Localhost Connection Refused (IPv4 Binding) | Environment | High |
...
## TS-049: Localhost Connection Refused (IPv4 Binding)
...
### Symptoms
Frontend fails to connect to backend with `ERR_CONNECTION_REFUSED` even though server is running. Vite HMR or API calls fail when using `localhost`.

### Root Cause
Vite or Node may bind to IPv6 (`::1`) by default on some Windows machines, while the client/proxy expects IPv4 (`127.0.0.1`). This mismatch causes the browser to reject the connection.

### Debug Steps
1. Check terminal output for Server/Client binding address.
2. Try accessing `http://127.0.0.1:5173` instead of `http://localhost:5173`.

### Solution
1. **Force IPv4 Binding**: Run vite with `--host 127.0.0.1`.
2. **Explicit Config**: In `vite.config.ts`, set `server.host: '127.0.0.1'`.
3. **Relative Proxy**: Use relative paths in client `.env` (e.g., `VITE_API_URL=/api/v1`) to let Vite's proxy handle the routing consistently.
