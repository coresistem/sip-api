...
export const getCart = async (): Promise<any> => {
    const response = await api.get('/jersey/cart');
    return response.data.data;
};

export const addToCart = async (productId: string, quantity: number = 1, size?: string): Promise<any> => {
    const response = await api.post('/jersey/cart', { productId, quantity, size });
    return response.data.data;
};
