...
export default function EventDashboard() {
    ...
    // Determine view mode
    const isEO = user?.role === 'EO' || user?.role === 'SUPER_ADMIN';
    ...
    // --- VIEW FOR ATHLETES / PUBLIC / CLUBS (Tournament Finder) ---
    if (!isEO) {
        return (
                ...
                >
                    <div>
                        <h1 className="text-2xl md:text-3xl font-display font-bold">
                            Find <span className="gradient-text">Tournaments</span>
                        </h1>
                        <p className="text-dark-400 mt-1">
                            Discover and register for upcoming archery events
                        </p>
                    </div>
                </motion.div>
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {events.map((event, index) => (
                        <motion.div
                            ...
                            className="card group overflow-hidden hover:border-primary-500/50 transition-colors"
                        >
                            {/* Placeholder Banner / Poster */}
                            <div className="h-40 bg-gradient-to-br from-dark-800 to-dark-900 relative flex items-center justify-center p-6">
                                <div className="absolute inset-0 bg-dark-950/20 group-hover:bg-transparent transition-colors" />
                                <Trophy className="w-16 h-16 text-dark-700 group-hover:text-primary-500/50 transition-colors" />
                                <div className="absolute top-4 right-4">
                                    <span className={`px-2 py-1 text-xs font-bold rounded ${getTypeBadge(event.type)}`}>
                                        {event.type}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="p-5 space-y-4">
                                <div>
                                    <h3 className="text-lg font-bold text-white leading-tight mb-2 group-hover:text-primary-400 transition-colors">{event.name}</h3>
                                ...
                                <div className="flex items-center justify-between py-3 border-y border-dark-700/50">
                                    <div className="text-center">
                                        <div className="text-xs text-dark-500 uppercase tracking-wider font-bold">Date</div>
                                    ...
                                    <div className="text-center">
                                        <div className="text-xs text-dark-500 uppercase tracking-wider font-bold">Participants</div>
                                        <div className="text-sm text-white font-medium mt-1">
                                            {event.participantCount} Archers
                                        </div>
                                    </div>
                                    <div className="w-px h-8 bg-dark-700/50" />
                                    <div className="text-center">
                                        <div className="text-xs text-dark-500 uppercase tracking-wider font-bold">Status</div>
                                        <span className={`text-xs font-bold mt-1 block ${event.status === 'OPEN_REGISTRATION' ? 'text-green-400' : 'text-dark-400'}`}>
                                            {event.status === 'OPEN_REGISTRATION' ? 'OPEN' : event.status}
                                        </span>
                                    </div>
                                </div>
    ...
    // --- VIEW FOR EO (Management Dashboard) ---
    return (
        <div className="space-y-6">
            {/* Header */}
            <motion.div
